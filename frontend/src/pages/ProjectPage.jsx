...
import { Plus, FolderKanban, Users, ArrowRight, Trash2, Settings } from 'lucide-react';
  ...
  <div onClick={onClick}
    ...
    <div className="flex items-start justify-between mb-3">
      <div className="flex items-center gap-2">
        <div className="w-10 h-10 rounded-xl flex items-center justify-center text-xl" style={{ background: project.color + '25' }}>
          {project.icon || '📁'}
        </div>
        <div>
    ...
    <div className="flex items-center justify-between">
      <AvatarGroup users={project.members?.map(m => m.user) || []} size="xs" max={4} />
...
const ProjectPage = () => {
  ...
  const [form, setForm] = useState({ name: '', description: '', workspace: '', color: PROJECT_COLORS[0], icon: '📁' });
  ...
  const handleCreate = async (e) => {
    e.preventDefault();
    if (!form.name.trim() || !form.workspace) {
    ...
    try {
      ...
      setShowModal(false);
      setForm({ name: '', description: '', workspace: workspaces[0]?._id || '', color: PROJECT_COLORS[0], icon: '📁' });
  ...
  const workspaceOptions = workspaces.map(w => ({ value: w._id, label: w.name }));
    ...
    <div>
      ...
      {projects.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-gray-500 mb-6 max-w-sm">Create your first project and start managing tasks with your team.</p>
          <button onClick={() => setShowModal(true)} className="btn-primary flex items-center gap-2">
      ...
      >
        <form className="space-y-4">
          ...
          {workspaceOptions.length > 0 && (
            <Select label="Workspace *" options={workspaceOptions} value={form.workspace}
      ...
      {deleteTarget && (
        ...
        >
          <p className="text-gray-300">Are you sure you want to delete <span className="font-semibold text-white">"{deleteTarget.name}"</span>? This will also delete all its tasks.</p>
        </Modal>
      )}
    </div>
  );
